...
public class PopulationPerAgeGroup extends IndicatorBase<PopulationPerAgeGroupRow> {
	...
	public void remove(HouseholdRow household) {
		for (PersonRow person : household.getMembers()) {
			...
			if (pos < 0) {
				...
				throw new AssertionError("PopulationPerAgeGroup.remove() - Spatial unit " + person.getHousehold().getDwelling().getSpatialunitId() + " & age group " + AgeGroup.getAgeGroupId(person.getAge()) + " are not in the list of spatial units");
			} else {
				// available at position pos
	...
	public int lookup(PersonRow person) {
		...
		lookup.setAgeGroupId(AgeGroup.getAgeGroupId(person.getAge()));
	...
	public void update(int pos, PersonRow person, boolean remove) {
		...
		if (remove)
			...
			b.setPersonCount(b.getPersonCount() + 1);
		assert b.getPersonCount() >= 0 : "Spatial unit " + person.getHousehold().getDwelling().getSpatialunitId() + " & age group " + AgeGroup.getAgeGroupId(person.getAge()) + " - person count < 0";
